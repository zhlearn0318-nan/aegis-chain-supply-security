# M5 P0/P1/P2 工程收敛执行计划（比赛交付范围已调整）

> 启动日期：2026-08-24  
> 执行分支：`dynamic-audit-v1`  
> 原始授权：完成 M5 全部 P0/P1/P2 工程项；若执行 P0-5，必须使用真实 Windows VM，不以目录或容器模拟替代。
>
> 2026-08-26 范围决策：赛题未把洁净 Windows VM 验收列为强制交付物；在三次真实运行失败并完成跨平台修复后，P0-5 延期为可选增强，不再阻断比赛交付。当前优先级转为可演示原型、量化效果、复现说明和提交材料。
...
| P0-5 发布门 | 延期；三次失败证据保留 | 真实 Windows VM 从私有远端新克隆到四链 E2E | 非比赛候选版前置；仅在核心交付完成且有余量时恢复 |
| P1 受控试点能力 | 部分/未完成 | 静态 worker 隔离、MCP/动态独立集、生命周期、CI、能力健康 | 按比赛得分贡献选择实施，不再要求交付前全部完成 |
| P2 生产化控制面 | 未完成 | 身份/RBAC/租户、持久队列与 DB、强沙箱、治理、完整准入、可观测性 | 作为生产化路线与限制，不作为本次比赛交付门 |
...
- 当时下一节点曾设为 P0-5 真实 Windows VM 验收；该要求只约束“若执行 P0-5 时不得模拟”，已被 2026-08-26 的比赛交付范围决策调整为延期项。
...
- 私有仓库认证已实现临时单仓库只读 Deploy Key 模式，强制严格主机密钥校验、禁交互、与 token 互斥，并在 clone 尝试结束后删除 guest 私钥；实际临时 Deploy Key 已用于三次 clone，并在 2026-08-26 吊销。
- 三次真实主运行均完成远端新克隆、VM 证明和负向 preflight，但分别暴露日志保真、Windows Conda 解释器布局和 Conda PATH 问题；对应修复及失败证据均已保留，第四次运行未执行。
- 当前决策：`deferred_non_blocking_for_contest_delivery`。P0-5 仍未完成，也不得声明通过；临时 GitHub Deploy Key 已吊销，本地密钥材料已删除。生产发布仍为 `NO-GO`，但比赛交付不再以该门为前置。
