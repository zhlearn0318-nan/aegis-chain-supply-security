...
结论：**静态审计工程开发及首轮高优先级加固已完成，进入一次性封存回归前的冻结状态；600 条回归集仍未打开。**
...
```text
Skill ZIP
  ├─ 安全解压、文件数/单文件与累计大小/压缩比/路径与特殊文件边界
  ├─ Cisco Skill Scanner
...
| E10 | MCP 能力策略 | 任意命令、无根目录文件能力、无限制 URL、通配权限、指令覆盖、敏感/明文 URI | 上传文字/字段只能 REVIEW；平台可信 sidecar 可 ALLOW；Platform v5 20/20 | 完成并加固 v1 |
...
### 5.2 Static Coverage v2 加固验证

- 120/120 产生覆盖摘要；
- 发现 1 个 `AEGIS_STATIC_PYTHON_PARSE_FAILED`；
- normal 升级、正确控制变化、旧层差异、哈希变化均为 0；
- `tests/fixtures` 安全文本单独计数，覆盖 INFO 不再被表述为安全结论；
- 回归集打开数为 0。

覆盖 Finding 的含义是“当前分析器不能证明已检查”，不是“已发现恶意”。这避免把跳过、解析失败或二进制载荷误报为安全。

### 5.3 Dependency / MCP Platform Controls v5

- 20/20 冻结微用例通过；安全哈希锁和平台可信 sidecar MCP 对照保持 ALLOW，仅有范围文字或用户上传边界字段的 MCP 进入 REVIEW；
- 重复分析确定性差异为 0；
...
- ZIP 上传：单成员展开不超过 15 MiB、累计不超过 50 MiB、压缩比不超过 200:1，并拒绝加密、链接、设备、重复目标和文件/目录冲突；
- MCP：每类最多 500 个对象、单 JSON 1 MiB；静态阶段不连接或调用 MCP Server；
- 依赖：manifest 最大 1 MiB、2,000 个逻辑行；不安装、不导入依赖；
- Finding ID 基于规范化证据确定性生成；Cisco Skill/MCP 原始对象名、描述、威胁摘要和代码片段不进入 Finding/SQLite，仅保留通用信号、脱敏对象 ID 与证据 SHA-256；
...
实验目录不可覆盖。失败轮次和存在剩余问题的冻结候选均保留，新校准使用新的 run ID。freeze v1 功能门通过但成功日志仍含内部临时路径；v2 清除了路径但 Cisco Skill 单结果报告的日志计数仍显示为0；v3同时验证最小留存与计数准确性；v4把后端、前端和生产构建验证纳入冻结产物并扩大哈希清单，但摘要标题仍写死为v1；v5修正自描述标题；v6封闭四项问题并通过既定自动门禁，但后续设计复核发现上传对象也能伪造机器边界字段，因此不作为最终候选；v7只接受平台调用方独立传入的可信 sidecar，但提交前发现混合 CRLF/LF 会使重新克隆后的源码哈希不一致；v8统一源文件和冻结产物为 LF，并保留全部功能门禁。

## 10. 当前冻结决策与下一道门

静态代码、规则、契约、加固门禁和开发证据满足 v8 冻结条件。下一道门只有一项：在用户明确授权后，对 600 条封存 Skill 回归集执行一次最终评估，产出独立的 normal 回退、恶意召回、F1/coverage、差分规则和哈希证据。运行前不再依据回归内容改规则；若结果失败，应如实报告并建立下一版本，不能回看后覆盖本次冻结。

因此，可向老师汇报：**静态审计开发及高优先级加固已完成并进入 v8 冻结候选；独立回归尚未执行，最终泛化指标待封存集揭盲。**
