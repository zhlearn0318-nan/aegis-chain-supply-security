...
扫描器进程禁止复制平台服务环境，只接收 Windows 启动兼容项、固定离线开关、受控 PATH、仓库临时目录和合成用户 profile。真实用户目录、云凭据、管理员令牌和认证代理均不传入。企业代理后续必须走管理员显式配置与凭据托管，不能恢复隐式全环境继承。

### 5.6 审计与部署前门

- 每次准入写入最小化 SQLite 记录，并以前序 SHA-256 形成可重算完整性链；不保存源码和绝对 sourcePath。
- 应用层 UPDATE/DELETE 被触发器拒绝；审计写入失败直接 block。
