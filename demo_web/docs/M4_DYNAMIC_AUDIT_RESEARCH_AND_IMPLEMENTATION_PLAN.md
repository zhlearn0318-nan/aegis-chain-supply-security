...
当前环境的 `docker` 命令不可用，因此本轮只执行仓库内自建、哈希锁定的良性 fixture。
...
- [ ] 安装/发现 Docker Desktop；
- [ ] 固定基础镜像与依赖；
- [ ] 实现只读输入、临时输出、默认断网、资源限制和清理；
...
D1 已由 `2026-08-22-dynamic-marker-flow-dev-v2` 完成受控机制验证：1/1 fixture、3/3 预期事件、1 条 Base64 源到汇 witness、`confirmed` 关联、原始 Marker 泄露和静态决策变化均为 0，完整后端测试 `270 passed`。下一步进入 D2 的 Docker 安全执行后端门；当前主机 `docker` 命令不可用，因此不得执行第三方样本。
