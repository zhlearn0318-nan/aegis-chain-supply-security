...
## M12 OpenClaw 统一安全中心正式发布

- [x] 五个独立侧边栏入口合并为唯一“Aegis 安全中心”。
- [x] 默认总览读取真实审计统计、链完整性和最近事件。
- [x] 准入、报告、审计、规则、MCP 通过统一六标签同层切换。
- [x] 保留旧 HTTP 路由，并跳转到安全中心对应功能。
- [x] 保持 `frame-ancestors 'self'`，未为页面合并放宽 CSP。
- [x] 修正规则页 `reset` ID 遮蔽表单原生方法的问题并加入回归。
- [x] 真实 Edge 验收：安全更新59–60、恶意阻断61，61条审计链有效。
- [x] Node `14 passed`；全模块导航控制台错误 0。
- [x] 一键安装器改为验证一个入口、六个视图并写入单入口回执。
- [ ] 第二台洁净 Windows/真实 VM 单入口版本验收（不阻断比赛交付）。

## P2 权威数据集与研究证据

- [x] 固定 MaliciousSkillBench 官方提交、许可边界与 Source-Disjoint test 协议。
- [x] 以全量 1,384 条替代二次抽样，保留 40 条完整包作为动态补充集。
- [x] 实现原始文件哈希、落地文本哈希、扫描标签隔离和失败关闭。
- [x] 完成 Cisco-only / Cisco+P0 / Cisco+P0+P1 三版本真实批量扫描。
- [x] 输出统一指标、配对置信区间、McNemar 检验、按来源/攻击类别和规则级误差。
- [ ] 在官方 train/validation 上复现轻量文本模型并冻结阈值。
- [ ] 完成语义、能力一致性、本地模型和 P1 动态路线消融。
- [x] 补充真实第三方配对集的逐案例静态/动态决策、规则命中、原始日志和综合准入差距材料。
...
- [x] 本阶段最终后端完整回归 `507 passed, 1 skipped`；跳过项仅为 Windows 符号链接权限。
- [ ] 可选：Falco/eBPF preflight 与内核级交叉证据；失败不阻断默认 Python 后端。
- [x] 冻结第三方动态实验合同：6个官方原始包、30个受控风险孪生、5类风险、三轮参数、阈值、停止条件和哈希证据。
- [x] 完成36包×3轮共108次真实容器调用；动态规则召回30/30、风险非放行30/30、原始脚本动态 ALLOW 6/6，安全/清理/输入不变均100%，残留0。
- [x] 已知恶意第三方 Skill 执行0；通过受控孪生证明动态增量，明确不把孪生风险归因于上游官方项目。
- [x] 完成静态—动态—单调融合后验分析：动态补齐4个静态 ALLOW 漏检、14个 REVIEW 升级 BLOCK；原始包综合仅1 ALLOW、4 REVIEW、1 BLOCK，静态误报治理进入下一路线。

## M15 E01–E06 实验计划

- [x] 新建 `experiment-m15` 隔离分支，保留 `main` 和 `v0.1` 不变。
- [x] 将 M15 六组实验计划纳入当前执行边界。
- [x] 冻结 E01 数据、S0–S6、轻量模型、统计与主接受门合同。
- [x] 生成并验证 train 7,513条与validation 835条标签隔离目录。
- [x] E01 v1 在validation前识别五折来源分组不可行并停止；冻结v2三折合同，未静默随机切分。
- [x] E01 v2 三折预检仍存在纯恶意折；冻结v3逐来源留一合同，保留两次负结果。
- [x] E01 v3 识别跨来源校准失败并在validation前停止；冻结v4断点与高阈值尾部合同。
- [x] E01 v4 主动发现训练预检越过标签文件边界并判无效；冻结v5严格标签盲预检合同。
- [x] 完成train逐来源留一、模型与阈值冻结（C=2.0，阈值0.99）。
- [x] 完成E01标签盲烟雾、Qwen冷启动修复和validation单次锁箱；S5主接受门通过。
- [x] 完成E02真实生态误报治理；v5通过真实40 Skill、SkillTrustBench regression600和MaliciousSkillBench train来源分组三道门，并接入正式静态准入流水线。
- [x] 完成E03动态0/1/3轮消融；严格前缀重放证明D3将风险非放行从73.3%提升至100%，五类风险均在adversarial轮首次触发，P95耗时比1.15，保留三轮默认。
- [x] 完成E04动态绕过鲁棒性；24组单因素配对、48个Skill、144次轮次执行中风险非放行与预期证据均24/24，安全对照误报0，三种运行时各8/8，全部安全/清理/哈希门通过。
- [x] 完成E05 OpenClaw端到端稳定性与性能：36/36 正常场景通过，非放行安装 0，并发保持单任务互斥；三类依赖故障均失败关闭，Docker Desktop 自动恢复仍为已知限制。
- [x] 完成E06误报、漏报与典型案例分析：分析60个分层漏报与全部54个良性非ALLOW，文本哈希验证100%，形成7项有样本支持的改进方向和8个唯一代表案例；未在同一锁箱回调规则。
- [x] 完成 M15 最终自动回归：正式运行时后端 `556 passed, 1 skipped`，OpenClaw 插件 `19 passed`；插件页面 HTTP 200、安装审计哈希链有效、Docker 29.7.2 可用。
