...
当前已完成 M1.3 的可配置准入策略、`/api/v1` 和前端 v1 适配，并完成 M2 的 SkillTrustBench v1.0 全量 5,520 条评测。M3 静态审计已在 600 条密封工程回归上完成一次性评估并冻结，结论为 `supported_with_tradeoff`。M4 已实现政企 Marker、静态 Trigger Plan、Docker 安全底座、受控 MCP 2025-06-18 stdio 调用，以及客户端侧 Linux inotify/procfs 独立遥测。M5 P0-1 至 P0-4 已完成：后端 `357 passed`，项目自身供应链门 12/12 通过，共享 Python 运行时与 Node 安装图已知漏洞均为 0。P0-5 真实 VM 验收程序已实现并通过本机非正式烟雾，真实 VM 证据仍未取得。动态部分仍只执行自建哈希锁定 fixture，不执行第三方 Skill/MCP。
