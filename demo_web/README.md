...
当前已完成 M1.3 的可配置准入策略、`/api/v1` 和前端 v1 适配，并完成 M2 的 SkillTrustBench v1.0 全量 5,520 条评测。M3 静态审计已在 600 条密封工程回归上完成一次性评估并冻结，结论为 `supported_with_tradeoff`。M4 已实现受控动态证据，M5 P0-1 至 P0-4 已完成。M6-3 已完成真实 OpenClaw 稳定版安装准入、扫描环境白名单、审计失败关闭、SQLite 完整性链和部署前检查；可确认 warn 与 Plugin/MCP 包准入仍未完成。后端 `390 passed`，生产判断保持 `NO-GO`。动态部分仍只执行自建哈希锁定 fixture，不执行第三方 Skill/MCP。
